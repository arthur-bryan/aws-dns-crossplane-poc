...
export class DockZoneEnrichmentProcessor implements CatalogProcessor {
  ...
  async preProcessEntity(
    ...
    if (specType !== SPEC_TYPE) return entity;
    ...
    if (status.nameServers && status.nameServers.length) {
    ...
    return {
      ...entity,
      metadata: { ...entity.metadata, annotations: next },
    };
  }
